...
public interface HistoryBankRepository extends JpaRepository<HistoryBank, Long> {
	...
	@Query(value = "SELECT hb.ID_HISTORY_BANK , hb.TANGGAL , hb.NOREK , hb.STATUS_KET , hb.NAMA ,hb.UANG ,hb.NOREK_DITUJU ,mb.NAMA ,hb.NO_TLP  FROM HISTORY_BANK hb JOIN MASTER_BANK mb \r\n"
			+ "ON (hb.NOREK_DITUJU  = mb.NOREK) WHERE hb.STATUS_KET = ?1", countQuery = "SELECT count(*) FROM HISTORY_BANK hb JOIN MASTER_BANK mb \r\n"
					+ "ON (hb.NOREK_DITUJU  = mb.NOREK) WHERE hb.STATUS_KET = ?1", nativeQuery = true)
	...
	@Query(value = "SELECT * FROM HISTORY_BANK WHERE :sortField LIKE '%' || :keyFilter || '%' ORDER BY :sortField :sortOrder", countQuery = "SELECT count(*) FROM HISTORY_BANK", nativeQuery = true)
	Page<HistoryBank> findAllFilter(@Param("keyFilter") String keyfilter, @Param("sortOrder") String sortOrder,
			@Param("sortField") String sortField, @Param("paging") Pageable paging);

	@Query(value = "SELECT * FROM HISTORY_BANK hb ORDER BY ID_HISTORY_BANK DESC ", nativeQuery = true)
	List<HistoryBank> findLastHistory();

	@Query(value = "SELECT * " + "FROM   HISTORY_BANK " + "WHERE  TANGGAL < TRUNC( SYSDATE ) "
			+ "AND  TANGGAL > TRUNC( SYSDATE ) - INTERVAL '1' DAY " + "AND  STATUS_KET = 2"
			+ "ORDER BY TANGGAL", nativeQuery = true)
	List<HistoryBank> newTarikDaily();

	@Query(value = "SELECT * " + "FROM   HISTORY_BANK " + "WHERE  TANGGAL < TRUNC( SYSDATE ) "
			+ "AND  TANGGAL > TRUNC( SYSDATE ) - INTERVAL '1' MONTH " + "AND  STATUS_KET = 2"
			+ "ORDER BY TANGGAL", nativeQuery = true)
	List<HistoryBank> newTarikMonthly();

	@Query(value = "SELECT * " + "FROM   HISTORY_BANK " + "WHERE  TANGGAL < TRUNC( SYSDATE ) "
			+ "AND  TANGGAL > TRUNC( SYSDATE ) - INTERVAL '7' DAY " + "AND  STATUS_KET = 2"
			+ "ORDER BY TANGGAL", nativeQuery = true)
	List<HistoryBank> newTarikWeekly();
...
//	 @Query(value = "SELECT COUNT(*) FROM HISTORY_BANK WHERE STATUS_KET=1 AND TANGGAL=TO_DATE(':hari','yyyy-MM-dd')", nativeQuery = true)
//	 Long adaSetoran (@Param("hari") String hari);
...
//	 @Query(value = "SELECT COUNT(*) FROM HISTORY_BANK WHERE STATUS_KET=1 AND TANGGAL BETWEEN TO_DATE(:start, 'yyyy-MM-dd') AND TO_DATE(:end, 'yyyy-MM-dd')", nativeQuery = true)
//	 Long adaSetorRekap (@Param("start") String start, @Param("end") String end);
