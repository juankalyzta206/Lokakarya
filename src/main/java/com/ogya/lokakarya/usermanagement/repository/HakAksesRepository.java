...
public interface HakAksesRepository extends JpaRepository<HakAkses, Long> {
	@Query(value = "SELECT COUNT(*) FROM USERS u WHERE u.USER_ID = :userId", nativeQuery = true)
	...
	@Query(value = "SELECT COUNT(*) FROM HAK_AKSES ha WHERE ha.USER_ID = :userId AND ha.ROLE_ID = :roleId", nativeQuery = true)
